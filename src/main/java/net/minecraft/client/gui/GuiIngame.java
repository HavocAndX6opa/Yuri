...
public class GuiIngame extends Gui {
    ...
    public void renderCustomScoreboard(ScoreObjective scoreObjective, ScaledResolution resolution) {
                ...
                if (hudModule != null && hudModule.isEnabled()) {
                    ...
                    if (ModListModule.moduleCache != null) {
                        for (Module module : ModListModule.moduleCache) {
                            if ((ModListModule.hideVisuals.getValue() && module.getCategory() == ModuleCategory.RENDER) ||
                                    (ModListModule.hideMisc.getValue() && module.getCategory() == ModuleCategory.MISC) ||
                    ...
                    if (visibleModules > 0) {
                        float arrayListBottomY = ModListModule.offset.getValue().floatValue() + (visibleModules * (8f + (ModListModule.padding.getValue().floatValue() * 2f)));
                        float desiredTopY = arrayListBottomY + 5f;
                        float defaultTopY = (resolution.getScaledHeight() / 2f) - (totalBoardHeight / 2f);

                        targetOffset = desiredTopY - defaultTopY;
                    }
                }
            }
        }
        ...
        if (useMcFont) {
            this.mc.fontRendererObj.drawStringWithShadow(header, headerX, boardTopY, textColor);
        } else {
        ...
        for (int i = 0; i < scoreCount; i++) {
            ...
            float playerX = startX + (maxWidth - playerNameWidth) / 2f;

            if (useMcFont) {
                this.mc.fontRendererObj.drawString(playerName, playerX, rowY, textColor, false);
            } else {
                customFr.drawString(playerName, playerX, (float) rowY, textColor);
            }
        }
    }
}
